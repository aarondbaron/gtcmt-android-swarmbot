...
public int fs;
  ...
  public void resetSample(boolean restart) {
    ...
    fperiod = (1f/p_base_freq)*fs; //CHANGED FOR STATIC COIN
    period = (int) fperiod;
  ...
  public double synthSample() {
    ...
    for (int si = 0; si < 8; si++) // 8x supersampling
    {
      ...
      //might need this
    	 int test=phase * 32 / period ;
    	 if(test<noise_buffer.length && test>0)
    	 sample = noise_buffer[test];    	
        break;
      }
  ...
  public void writeValueStereo(byte[] target, int position,
