...
public:
	void visionTrack(){
		...
		Int32 port = 8080;
		IPAddress^ localAddr = IPAddress::Parse( "143.215.97.218" );
...
//		connected=true;
		Console::WriteLine( "Connected to robot 0" );
			...
			if( !paused )
			{
				...
				if( trackObject )
				{
					...
					if(trackBox.size.height>=0&&trackBox.size.width>=0){
						...
						{
							cout<<"exception caught"<<endl;//<e->GetType()->Name<
							cout<<"lose connect to robot 0"<<endl;
							client = server->AcceptTcpClient();
							stream = client->GetStream();
							cout<<"Connected to robot 0"<<endl;
						}
						try{
						...
						catch(IOException^ e)
						{
							...
							cout<<"lose connect to robot 1"<<endl;
							client1 = server->AcceptTcpClient();
							stream1 = client1->GetStream();
							cout<<"Connected to robot 1"<<endl;
						}
...
//							printf("OBJ2 %d\t%d\n",trackWindow2.width,trackWindow2.height);
	...
	void controller(){
		...
		while(1)
		{
			...
			switch(data[0]){
				case 'w': send="forward1";	robot=1;	break;
				case 'a': send="rotleft1";	robot=1;	break;
				case 's': send="backward1";	robot=1;	break;
				case 'd': send="rotright1"; robot=1;	break;
				case 'q': send="stop1";		robot=1;	break;
				case 'i': send="forward2";	robot =2;	break;
				case 'j': send="rotleft2";	robot =2;	break;
				case 'k': send="rotright2";	robot =2;	break;
				case 'l': send="backward2";	robot =2;	break;
				case 'u': send="stop2";		robot =2;	break;
				default:	send= data;		robot=0;	break;	
			}
